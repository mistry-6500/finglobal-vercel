...
function Archive() {
  ...
  const datedResults = [
    ...results.map((article) => ({ kind: "curated" as const, date: article.published, article })),
    ...liveResults.map((item) => ({ kind: "live" as const, date: item.published, item })),
  ...
  return <div className="mx-auto max-w-6xl px-4 py-10">
    ...
    {datedResults.length ? <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{datedResults.map((entry) => entry.kind === "curated" ? <article key={entry.article.slug} className="rounded-lg border border-border bg-card p-5 transition-colors hover:border-primary"><p className="eyebrow text-primary">{marketName(entry.article.market)} · {entry.article.topic}</p><time dateTime={entry.article.published} className="mt-2 block font-mono text-xs text-muted-foreground">{entry.article.published}</time><h2 className="mt-2 text-xl font-semibold text-balance"><Link to="/news/$slug" params={{ slug: entry.article.slug }} className="hover:text-primary">{entry.article.title}</Link></h2><p className="mt-2 line-clamp-3 text-sm text-muted-foreground">{entry.article.standfirst}</p><Link to="/news/$slug" params={{ slug: entry.article.slug }} className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-primary">Read story <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></article> : <article key={`live-${entry.item.id}`} className="rounded-lg border border-border bg-card p-5 transition-colors hover:border-primary"><p className="eyebrow text-primary">{marketName(entry.item.market)} · {entry.item.topic}</p><time dateTime={entry.item.published} className="mt-2 block font-mono text-xs text-muted-foreground">{entry.item.published.slice(0, 10)}</time><h2 className="mt-2 text-xl font-semibold text-balance"><Link to="/news/$slug" params={{ slug: entry.item.id }} className="hover:text-primary">{entry.item.title}</Link></h2><p className="mt-2 line-clamp-3 text-sm text-muted-foreground">{entry.item.summary || "Live coverage with context from the FinWorldNews desk."}</p><Link to="/news/$slug" params={{ slug: entry.item.id }} className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-primary">Read story <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></article>)}</div> : <div className="mt-4 rounded-lg border border-dashed border-border p-10 text-center"><h2 className="text-xl font-semibold">No stories match those filters</h2><p className="mt-2 text-sm text-muted-foreground">Try a broader search or return to the full archive.</p><Link to="/archive" className="mt-4 inline-block text-sm font-semibold text-primary hover:underline">Clear filters</Link></div>}
  </div>;
}
